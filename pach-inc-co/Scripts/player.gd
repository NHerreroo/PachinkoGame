...
# ═══════════════════════════════════════════════════════
func _ready() -> void:
	# Conectar señales de la cámara si es necesario
	pass
...
func _physics_process(delta: float) -> void:
	...
	# Actualizar sway de la cámara
	var move_dir_local := Vector3(input_dir.x, 0.0, input_dir.y).normalized()
	camera_controller.update_sway(move_dir_local if is_moving else Vector3.ZERO, delta)
	...
	if held_object != null:
		var target_pos = camera_controller.get_hand_position() + bob_offset
		...
		var base_target_quat = camera_controller.get_hand_rotation().get_rotation_quaternion()
		var target_quat      = base_target_quat * Quaternion.from_euler(camera_controller.get_sway_rotation())
	...
	if held_object == null and camera_controller.is_pickable_in_range():
		var collider = camera_controller.get_pickable_object()
		if collider != null and Input.is_action_just_pressed("Pick"):
...
func _update_crosshair(delta: float) -> void:
	var can_pick = (
		held_object == null
		and camera_controller.is_pickable_in_range()
	)
...
func _update_highlight(delta: float) -> void:
	...
	if camera_controller.is_pickable_in_range() and held_object == null:
		new_target = camera_controller.get_pickable_object()
...
func drop_object() -> void:
	...
	dropped.angular_velocity = camera_controller.get_angular_velocity() * DROP_ANGULAR_INHERIT
...
func throw_object() -> void:
	...
	var direction = camera_controller.get_forward_direction()

	_restore_object_physics(thrown)
	thrown.apply_central_impulse(direction * force)
	thrown.apply_torque_impulse(camera_controller.global_transform.basis.x * force * THROW_ANGULAR_MULT)
